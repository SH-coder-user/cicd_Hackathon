...
public class StockController {
    ...
    public ResponseEntity<?> expiring(@RequestParam(defaultValue="3") int days,
        ...
        var items = docs.stream().map(DocumentSnapshot::getData).toList();

        String nextCursor = docs.isEmpty() ? null : docs.get(docs.size()-1).getId();

        Map<String, Object> body = new java.util.HashMap<>();
        body.put("items", items);
        if (nextCursor != null) body.put("nextCursor", nextCursor); // null이면 넣지 않음
